...
- **投稿流程指引**：识别“帮我上传 ScanSci”，选择成稿并整理标题、分类与标签，复用已确认的署名和许可。登录、提交和回执规则见 [上传到 ScanSci](skill/references/scansci-upload.md)；当前线上投稿及客户端授权待接通。
...
生成满意的作品后，可以继续说“帮我把这张 SVG 上传 ScanSci”。Skill 会按当前服务能力准备或提交成稿；服务开通并完成首次登录授权后，后续可复用有效授权。上传成功、等待审核和已经公开会分别报告。
